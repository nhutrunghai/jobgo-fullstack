        ...
        <section className="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
          ...
          {logs.map((log) => (
            <article key={log._id} className={`border-t border-slate-100 px-4 py-3 text-[12px] transition lg:grid lg:grid-cols-[160px_1.2fr_150px_110px_150px_96px] lg:items-center lg:gap-3 ${selectedLog?._id === log._id ? 'bg-indigo-50/60' : 'hover:bg-slate-50'}`}>
              ...
              <div className="mt-3 lg:mt-0"><span className={`inline-flex rounded-full border px-2 py-1 text-[11px] font-extrabold ${log.success ? 'border-emerald-100 bg-emerald-50 text-emerald-700' : 'border-rose-100 bg-rose-50 text-rose-700'}`}>{log.success ? 'Th?nh c?ng' : 'Th?t b?i'}</span></div>
          ...
          {selectedLog ? (
            <div className="p-4">
              <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                ...
                <div className="rounded-md border border-slate-100 bg-slate-50 p-2.5"><p className="text-[10px] font-extrabold uppercase tracking-[0.15em] text-slate-400">???ng d?n</p><p className="mt-1 text-[12px] font-bold text-slate-800 break-all">{selectedLog.path || 'Chưa có'}</p></div>
                <div className="rounded-md border border-slate-100 bg-slate-50 p-2.5"><p className="text-[10px] font-extrabold uppercase tracking-[0.15em] text-slate-400">Ph??ng th?c / tr?ng th?i</p><p className="mt-1 text-[12px] font-bold text-slate-800">{selectedLog.method || 'N/A'} · {selectedLog.status_code || 'N/A'}</p></div>
